...
class ForecastCONVAeroTrainerClass(LSTMTrainerClass):

  def __init__(self):
    super(ForecastCONVAeroTrainerClass, self).__init__()
 ...
 # 
  ...
  def plot_predict(self,show=True,param="T",save=False):
    ...
    ax1.plot( predicted[1][0], label="Predicted начало тумана " )
    # _y = np.reshape(self.y_train,(-1,3))
    # print(_y[:,1])
    # print(_y[:,2])
    _train = self._scalerY[1].inverse_transform(self.y_train_2)
    ax1.plot( _train, label="Real value of начало тумана" )
    ...
    if save:
      ...
      # считаем берлянда
      # находим строки с данными за сроки 12 и 18 часов
      # print(m.loc[ (m.index.hour==12) & (m.index.hour==18) ])
      # exit()
      # забираем температуру и точку росы за -0, -6, -12, -18 часов
      # забираем час восхода солнца для этой даты и координат (sunrise) и захода (sunset) заодно
      # прогнозируем и для времени от захода до восхода +1ч следующего дня ставим результат прогноза (0/1)
      # 
      ...
      # print(len(m['opravd']),'m')
      m['zverev']       = self.calc_zverev(z)
      m['z_opravd']     = m.apply( lambda row: 1 if row['yes'] == 1 and row['zverev']==1 else 0 ,axis=1 )
      m['z_opravd_no']  = m.apply( lambda row: 1 if row['yes'] == 0 and row['zverev']==0 else 0 ,axis=1 )
      ...
      z_opravd    = m['z_opravd'].sum()
      z_opravd_no = m['z_opravd_no'].sum()

      m['Name'].iloc[11] = 'Uz'
      m['perc_opravd'].iloc[11] = (z_opravd+z_opravd_no)/rows * 100

      m['Name'].iloc[12] = 'Uz with'
      m['perc_opravd'].iloc[12] = z_opravd/z_yes * 100

      m['Name'].iloc[13] = 'Uz wout'
      m['perc_opravd'].iloc[13] = z_opravd_no/z_no * 100
      # 
      # =====
      # 
      ...
      m1 = (p_yes+yes)/rows
      m2 = (p_no+no)/rows
      # оправдываемость случаных прогнозов
      u0 = ( m1 + m2 ) / rows
      # критерий Багрова
      m['Name'].iloc[8] = 'Bagrov (H) (>0.33)'
      m['perc_opravd'].iloc[8] = (U - u0)/(1 - u0)
  ...
  # 
  # ситчаем методом зеверева
  # 
  def calc_zverev(self, df):
    df['zverev'] = 0
    # df.set_index( pd.to_datetime(df['date']) )
    # print(df)
    df_h = df[ ((df.index.hour==19) ) & (df.index.minute==0) ]

    # zverev
    for (index) in pd.unique(df_h.index):
      index = index.astype('M8[ms]').astype('O')
      ...
      sun     = Sun()
      sr = sun.getSunriseTimeDate({'longitude' : lon, 'latitude' : lat }, [ index.day, index.month, index.year ])
      if ( 'decimal' in sr.keys() ):
        sunrise = sr['hr']
      sn = sun.getSunsetTimeDate({'longitude' : lon, 'latitude' : lat }, [ index.day, index.month, index.year ])
      if ( 'decimal' in sn.keys() ):
        sunset  = sn['hr']

      # забираем температуру и точку росы во время захода солнца
      # 
      indx = index.replace(hour=int(sunset))
      ...
      # print(srok, ff, 5, 1, Tz, Dz, Dis, Tis)
      if zverev( srok, ff, 5, 1, Tz, Dz, Dis, Tis):
        to = index+timedelta( hours=sunrise+(24-19) )
        df.loc[index:to]['zverev']=1

    x_sz = (self.X_train.shape[0]//self._batch_size)*self._batch_size

    print(len(df['zverev'][0:x_sz].values))
    # 
    # возвращаем df и количество ояпов и без ояпов
    # 
    return df['zverev'][0:x_sz].values